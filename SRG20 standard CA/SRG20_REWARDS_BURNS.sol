...
contract SRG20 is IERC20, Context, Ownable, ReentrancyGuard {
    event Bought(
    ...
    //Tax wallets
    address public teamWallet = 0xF3184828Aa32A7fD77a3c09F700a73819Bb7ca7E;
    address public treasuryWallet = 0xF3184828Aa32A7fD77a3c09F700a73819Bb7ca7E;

    // Tax Split
    uint256 public teamShare = 0;
    uint256 public treasuryShare = 200;
    ...
    //trading parameters
    uint256 public liquidity = 5 * 10**5 * 10**_srgDecimals;
    ...
    function _buy(
        ...
        //burns
        if (!isFeeExempt[buyer] && burnShare>0) {
        bool temp1 = SRGIE.transfer(DEAD, (taxAmount * burnShare) /
                    SHAREDIVISOR);
        require(temp1, "Failed to burn SRG");
        taxBalance -= (taxAmount * burnShare) / SHAREDIVISOR;
        }
    ...
    function _sell(
        ...
        if (isFeeExempt[msg.sender]) {
            ...
            //burns
            if (!isFeeExempt[seller] && burnShare>0) {
                bool temp1 = SRGIE.transfer(DEAD, (amountTax * burnShare) /
                    SHAREDIVISOR);
                require(temp1, "Failed to burn SRG");
                taxBalance -= (amountTax * burnShare) / SHAREDIVISOR;
            }
    ...
    function changeTaxDistribution(
        ...
        require(
            newteamShare + newtreasuryShare + newBurnShare + newRewardShare ==
                SHAREDIVISOR,
